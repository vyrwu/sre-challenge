...
Code challenge hand-out during the recruitment process of Aleksander Nowak, for the position of Site Reliability Engineer/DevOps Engineer.
Thank you for taking the time to review my submission. Should you have any questions, please feel free to reach out!

An overall diagram of the solution:
...
Note that it may upgrade versions of some of the binaries to the latest versions.
...
You can control local Pulumi stacks individually using the provided NPM scripts under `invoice-app/iac/package.json` and `payment-provider/iac/package.json`.
```sh
...
Occasionally, the network exposed by the VM that runs Minikube will be unreachable from the host and will result in tests failing. Note that
...
pkill kubectl // Terminates the background port-forward
```
...
I provided a simple setup script that installs all necessary binaries and configures a local environment running in Minikube
from scratch.
...
Running pods as non-root is a good security practice, but also a common source of trouble with running containers
inside Kubernetes. I luckily suspected this might be the bug right from the beginning. After I deployed apps
to Kubernetes, they were crash looping (with a surprisingly expressive error message), so I could start solving the problem.
In this case, the app did not require any root access, so Dockerfile was changed to create a new non-root user and
switch to it before running the app binary. Additionally, I slimmed down the image size by removing unnecessary build
...
Regarding 2.1, I deployed both apps to Kubernetes using Pulumi IAC. I considered installing a local ArgoCD instance to do K8s GitOps, however,
...
right from the start. It was also a tool I was by far the most comfortable with.

Regarding 2.2 and 2.3, I added an `nginx-ingress-controller`, and gave `invoice-app` an Ingress, making that service reachable
from outside of the cluster. I encountered issues with reaching the Minikube network inside Docker, so I decided to run it in a
Hyperkit VM instead.
...
Regarding 2.6, all setups required to deploy a local development environment for the apps were automated using Bash scripts,
and the Makefile was updated to expose options to execute those scripts. All scripts now reside under the `scripts/`
directory. Documentation inside `README.md` gives instructions on how to set up a local development environment.

Regarding 2.7, I created a minimal set of integration tests in Go under `test/integration/invoices`. Those tests verify that
the Invoices API, exposed via Ingress, works as expected. Tests executing against the local environment are run with
...
and, via Kubernetes RBAC, introduced roles limiting developers’ actions based on the role they assume via AWS IAM. This
can also be achieved using simple User accounts with rotating credentials, but it is often not a compliant solution.

Regarding 3.4, to prevent other applications running in the cluster to communicate with `payment-provider`, but still allowing
...
explicitly granted, and other applications can block all traffic by default, only allowing the expected traffic through.

Regarding 3.1, I would like to preface my thoughts saying that I do not believe this setup to be the only correct setup.
...
effective for larger scale, but resulted in a heavy operational burned and long provisioning times on our
CI/CD pipelines. We created a certain bottleneck for software delivery, and gate-kept innovation as anything infrastructure-related
must have been developed as part of the SRE platform first. I currently firmly believe that teams should be as
autonomous as possible. However, expertise scales, so the SRE team should deliver easy-to-use solutions that utilise
...
but at the end of the day, developers should own the entire solution (You-Built-It-You-Run-It). The Embedded SRE model is perfect
for auditing the product team's software, advocating for SRE principles, sharing knowledge, and solving more challenging operational
